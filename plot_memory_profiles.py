...
def plot_metrics(best_data, next_data, args: argparse.Namespace):
    ...
    axs[1].plot(best_bytes_requested, best_lhs_fragmentation, label="Best Fit - Watermarking Cons. Fragmentation", marker=',', color='red')
    if args.separate_axis:
        ax2 = axs[1].twinx()
        color="tab:blue"
        ax2.tick_params(axis="y", labelcolor=color)
        ax2.set_ylabel('Next Fit - Watermarking Cons. Fragmentation')
        ax2.plot(next_bytes_requested, next_lhs_fragmentation, label="Next Fit - Watermarking Cons. Fragmentation", marker=',', color='red', linestyle="dotted")
        axs[1].set_ylabel('Best Fit - Watermarking Cons. Fragmentation')
    else:
        axs[1].plot(next_bytes_requested, next_lhs_fragmentation, label="Next Fit - Watermarking Cons. Fragmentation", marker=',', color='red', linestyle="dotted")
    ...
    axs[2].plot(best_bytes_requested, best_in_between_fragmentation, label="Best Fit - Alignment Cons. Fragmentation", marker=',', color='green')
    if args.separate_axis:
        ax2 = axs[2].twinx()
        color="tab:blue"
        ax2.tick_params(axis="y", labelcolor=color)
        ax2.set_ylabel('Next Fit - Alignment Cons. Fragmentation')
        ax2.plot(next_bytes_requested, next_in_between_fragmentation, label="Next Fit - Alignment Cons. Fragmentation", marker=',', color='green', linestyle="dotted")
        axs[2].set_ylabel('Best Fit - Alignment Cons. Fragmentation')
    else:
        axs[2].plot(next_bytes_requested, next_in_between_fragmentation, label="Next Fit - Alignment Cons. Fragmentation", marker=',', color='green', linestyle="dotted")
