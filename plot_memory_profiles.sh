...
echo "Plotting realistic workload (SEPARATE AXIS) run results: Standalone Application profiles.."
OUTPUT_DIR="./plots/memory_profiles/applications_standalone"
APP_NAMES=(hello timer mltest fibonacci)
...
do
    echo "App: $app"
    python3 plot_memory_profiles.py --best_fit_log_file best_fit/apps_standalone/apps_standalone_best_fit_app_name_$app.log --next_fit_log_file next_fit/apps_standalone/apps_standalone_next_fit_app_name_$app.log --output_dir $OUTPUT_DIR --separate_axis
done

echo "Plotting realistic workload (SEPARATE AXIS) run results: Sequential Application profiles.."
OUTPUT_DIR="./plots/memory_profiles/applications_sequential"
# Sequence 1: # 4 ramps/spiking
# Sequence 2: # single large ramp
# Sequence 3: # 3 start, 2 stop, 1 start, 2 stop
...
do
    echo "Sequence: $seq"
    python3 plot_memory_profiles.py --best_fit_log_file best_fit/apps_sequential/apps_sequential_best_fit_sequence_$seq.log --next_fit_log_file next_fit/apps_sequential/apps_sequential_next_fit_sequence_$seq.log --output_dir $OUTPUT_DIR --separate_axis
done
echo "Done :)"
